...
If you create a unique index for a column it means you're guaranteed the table won't have more than one row with the same value for that column. Using only validates_uniqness_of validation in your model is not enough to enforce uniqueness because there can be concurrent users creating invalid records. Therefore you need to create a unique constraint at the database level as well. Typical columns that should have unique indexes are username or e-mail for logins.
